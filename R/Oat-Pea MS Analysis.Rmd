...
Analysis for the oat-pea polyculture experiment. This experiment
compared the performance of oat, pea, and oat-pea in a mix. Performance
was measured by yield, biomass, and stability. The experiment was
planted on two dates, late April and June 1st, to simulate site-years.
The early planting date included a 3-way, oat-pea-flax mixture.

Major questions are:

1.  Does the oat-pea mixture yield more per acre than monocrops? This
    will be assessed by calculating land equivalent ratios.
2.  Is the oat-pea mixture more stable across weather patterns than the
    monocrops? This will be assessed as change in yield across planting
    dates.
3.  Does the addition of flax hinder oat and pea development?

The design is split plot with four blocks. The main plots are planting
date with crop mixture as subplots. Therefore, the error structure is
`~(1|BLOCK) + (1|BLOCK:PLANTING) + residuals` (error terms for block and
planting date within block).
...
for (i in libs) {
  if (!require(i, character.only=TRUE)) {
    renv::install(i, prompt=FALSE)
    library(i, character.only=TRUE)
  }
}

renv::snapshot(packages=libs)
```
...
make_anova_table = function(x, append_to=NULL) {
  av = x
  resp = attr(av, "heading")[2]
  resp = strsplit(resp, ": ")[[1]][2]
  
  
  p_col = grepl('Pr\\(>', colnames(av))
  out = as.data.frame(av[p_col]) # to allow using merge()
  names(out) = resp
  rownames(out) = rownames(av)
    
  if (!is.null(append_to)) {
    out = merge(append_to,        # robust to different lengths
                out, 
                by='row.names', 
                all=TRUE)
...
make_anova_table = function(x, append_to=NULL) {
  ...
  p_col = grepl('Pr\\(>', colnames(av))
  out = data.frame(response = resp,
                   predictor = rownames(av),
                   test = colnames(av)[1],
                   stat = av[,1], 
                   p_val = av[,p_col])
...
Missing some height data (late planting). Others are consistent with
experimental design.
...
-   Weed biomass, which is to be expected due to patchiness.That
    specific part of the field was especially weedy.
-   Predation seems bimodal, so tough to analyze.
-   Pea harvest index - late planting, oat-pea, replicate A. This is
    worth investigating/re-measuring.
...
```{r message=FALSE, warning=FALSE}
mod = lmer(ff, data=df)
av = Anova(mod, test.statistic='F')
av
```


A marginal interaction between crop and planting date. Crop has a main
effect, as does planting.

Save into an anova table for the manuscript. 
```{r warning=FALSE}
anova_table = make_anova_table(av)
```
...
In general, oats yielded better than mixes and peas. The late planting
was more productive - especially for the monocultures. For the early
planting, the mix seemed as productive as the oat monoculture. Whether
flax affected oat-pea yields is uncertain.

Yields were quite poor in all cases. Will need to compare to oat-clover
yields.

This suggests that the oat-pea mix is more stable across weather
patterns.
...
No interaction between crop and planting date. Crop has a main effect,
as does planting.

Save into an anova table for the manuscript. 
```{r warning=FALSE}
anova_table = make_anova_table(av, append_to=anova_table)
```
...
scatterplot approach. Points above the line are overyielding. So the
early planting date overyielded.
...
On average LER no different from 1, surprisingly. In the early planting
date it was a small increase and noise was such that we didn't see any
strong differences.
...
oat and pea yields differed across planting dates, while oat-pea yield
was not different.
...
Oat and pea yields were higher in the late planting than the early.
Oat-pea was not significantly different between plantings.
...
For oats, A marginal planting date effect. Most is an interaction
between mix and planting date.
...
In the mix, pea did better at the later planting date, while oats did
slightly better at the earlier planting date.
...
Pea produced more protein in the late planting due to higher yields
despite lower protein concentration. Oat-pea showed no significant
difference.
...
All crops produced similar amounts of protein across planting dates,
despite the very different grain ratios of the oat-pea mix.
...
All crops produced similar amounts of protein across planting dates,
despite the very different grain ratios of the oat-pea mix. \###
Comparisions *Planting date within crop*
...
Eggs in oat-pea-flax had a 73% chance of being eaten. Oat was about 1/3
that.
...
Marginal effect - pea had slightly lower weed biomass, but not
meaningfully so.
...
# format responses
is_predation = grepl('PREDATION', avt[, 'response'])
avt[is_predation, 'response'] = 'EGG_PREDATION'

is_weeds = grepl('WEED_BIOMASS', avt[, 'response'])
avt[is_weeds, 'response'] = 'WEED_BIOMASS'

is_biomass = grepl('COMBO_BIOMASS', avt[, 'response']) 
avt[is_biomass, 'response'] = 'CROP_BIOMASS'

avt[, 'response'] = avt[, 'response'] |>
  gsub('COMBO_', '', x=_) |>
  gsub('_HAND', '', x=_) |>
  gsub('_MATURITY', '', x=_)


avt[, 'response'] = strsplit(avt[, 'response'], '_') |>
  sapply(function(x) {
    if (length(x) > 1 | (length(x) == 1 & nchar(x[1]) > 3)) {
      x = sapply(x, function(y) {
        first = substr(y, 1, 1)
        rest = substr(y, 2, nchar(y))
        paste0(first, 
               tolower(rest))
      }) 
    }
...
facts = avt[!duplicated(avt[, 'predictor']), 'predictor']
avt[, 'predictor'] = factor(avt[, 'predictor'], 
                            levels=facts)
...
```{r}
avt = pivot_longer(avt, 
                   cols=where('is.numeric'))
avt = avt[avt$name=='p_val', ]
# avt[, 'value'] = 
#   sapply(avt[['value']],
#          function(x) {
#            if (is.na(x)) {
...
avt = pivot_wider(avt, id_cols=c('response'),
...
anova_gtab = gt(avt, 
  ...
  tab_header(md("**Table 2**: *p*-values of treatment effects on measured variables")) |>
  opt_align_table_header(align='left') |>
  tab_footnote("Abbreviations: CP - Crude Protein; TDN - Total Digestible Nutrients; ADF - Acid Detergent Fiber; NDF - Neutral Detergent Fiber") |>
  cols_align('center')
anova_gtab
...
```{r}
gtsave(anova_gtab,
       filename=here('Results', 'Table 2 - ANOVA.html'))
```
